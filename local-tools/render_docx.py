...
def convert_to_pdf(
    ...
    try:
        ...
        if os.name != "nt":
            raise
        # The project workstation has Microsoft Word but no LibreOffice.
        # Keep the canonical rasterization path and use Word only for the
        # format-conversion step, with an invisible, read-only document.
        ps_doc_path = str(Path(doc_path).resolve()).replace("'", "''")
        ps_pdf_path = str(Path(pdf_path).resolve()).replace("'", "''")
        word_script = (
            ...
            f"$doc=$word.Documents.Open('{ps_doc_path}');"
            f"$doc.ExportAsFixedFormat('{ps_pdf_path}',17);"
            "} finally {"
        ...
        _log_result("DOCX->PDF (Microsoft Word COM fallback)", cmd_word, proc)
